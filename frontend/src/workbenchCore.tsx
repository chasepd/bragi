...
function settingsPath(activeSaveId: string | null) {
  return activeSaveId
    ? `/api/settings?save_id=${encodeURIComponent(activeSaveId)}`
    : "/api/settings";
}

const SETTINGS_BACKGROUND_STALE_MS = 60_000;

function settingsQueryKey(activeSaveId: string | null) {
  return ["settings", "full", activeSaveId] as const;
}

function settingsQueryOptions(activeSaveId: string | null) {
  return {
    queryKey: settingsQueryKey(activeSaveId),
    queryFn: ({ signal }: { signal: AbortSignal }) => apiRead<SettingsModel>(settingsPath(activeSaveId), signal),
    staleTime: SETTINGS_BACKGROUND_STALE_MS
  };
}
