...
def test_repository_rejects_base_scenario_cas_after_save_override(
    repositories: PersistenceRepositories,
) -> None:
    scenario = repositories.create_scenario(
        type="full_roleplay",
    ...
    repositories.add_save_scenario_update(
        ...
        premise=scenario.premise,
        player_role=scenario.player_role,
        content={"lore": "The save-specific beacon burns blue."},
...
def test_repository_rejects_cas_for_superseded_active_scenario_update(
    repositories: PersistenceRepositories,
) -> None:
    scenario = repositories.create_scenario(
        type="full_roleplay",
    ...
    save = repositories.create_save(scenario_id=scenario.id, title="Night Watch")
    superseded = repositories.add_save_scenario_update(
        save_id=save.id,
        title=scenario.title,
        premise=scenario.premise,
        player_role=scenario.player_role,
        content={"lore": "The beacon burns blue."},
        reason="The lens first changed.",
        provider="fake",
        model="fake-context",
    )
    latest = repositories.add_save_scenario_update(
        save_id=save.id,
        title=scenario.title,
        premise=scenario.premise,
        player_role=scenario.player_role,
        content={"lore": "The beacon burns gold."},
        reason="The lens changed again.",
        provider="fake",
        model="fake-context",
    )
    ...
    assert not repositories.compare_and_set_active_save_scenario_content(
        save_id=save.id,
        update_id=superseded.id,
        expected_content_json=superseded.content_json,
        content={"lore": "Stale overwrite."},
    )

    refreshed = repositories.get_active_save_scenario_update(save.id)
    assert refreshed is not None
    assert refreshed.id == latest.id
    assert _load_content(refreshed.content_json) == {
        "lore": "The beacon burns gold.",
    }
