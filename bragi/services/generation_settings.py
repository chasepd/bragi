...
DEFAULT_CHAT_MAX_OUTPUT_TOKENS = 1200
MIN_CHAT_MAX_OUTPUT_TOKENS = 64
MAX_CHAT_MAX_OUTPUT_TOKENS = 4096
...
def model_thinking_reasoning_config(
    ...
    *,
    ...
    if level == THINKING_LEVEL_OFF:
        return None if bool(support.get("mandatory")) else ChatReasoningConfig(
            enabled=False,
            exclude=True,
        )
...
def model_thinking_preference_level(
    ...
    *,
    ...
    if support is None:
        return THINKING_LEVEL_PROVIDER_DEFAULT
    level = preference["level"]
    if level == THINKING_LEVEL_OFF and not bool(support.get("mandatory")):
        return level
